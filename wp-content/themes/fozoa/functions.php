...
add_image_size( 'blog-list-thumb', 158, 120, true );//Thumbnail Imprensa Post Type
add_image_size( 'faleconosco-thumb', 307, 371, true );//Thumbnail Fale conosco imagem do tema
add_image_size( 'temadepagina-tema-thumb', 406, 490, true );//Thumbnail  Fique por dentro imagem do tema
add_image_size( 'boxlink-tema-thumb', 315, 181, true );//Thumbnail  Fique por dentro imagem do tema
/*
...
if (!function_exists('get_menu')){
  function get_menu($page)
  {
    ...
    $menu = "";
    switch ($page) {
      case ($page=='residuos' || $page=='segmentos-de-atuacao' || $page=='servicos-prestados'):
        $menu = "residuos";
        break;
      case ($page=='utilities' || $page=='modelos-de-negocios' || $page=='unidades-utilities'):
        $menu = 'utilities';
        break;
      case ($page=='sustentabilidade' || $page=='meio-ambiente' || $page=='saude-e-seguranca-do-trabalho'):
        $menu = 'sustentabilidade';
        break;
      case ($page=='relacao-com-investidores' || $page=='graficos-de-participacao' || $page=='demonstrativos-financeiros'):
...
//Rotina valida o nome da página que deve exibir o Styled Header
if (!function_exists('showStyledHeader')){
  function showStyledHeader($page = "")
  {

    $retorno = FALSE;
    $pagina = array (
        'residuos', 'utilities', 'modelos-de-negocios', 'agua-e-esgoto'
    );
...
//Retorna o mes reduzido conforme o numero do mes passado como parametro
if (!function_exists('get_short_mes')){
  function get_short_mes($mes = 1)
  {

  $array = array("jan" => 0, "fev" => 1, "mar" => 2, "abr" =>3, "mai" =>4, "jun" =>5, "jul" => 6, "ago" => 7, "set" =>8, "out" =>9, "nov" =>10, "dez" => 11);

  $mes = array();
  $mes = array_keys($array, 1);
  return $mes[0];

  }
}

// Inclui uma classe específica para o primeiro e último item
if (!function_exists('nav_menu_first_last')){
...
if (!function_exists('short_text')){
  function short_text($after = '', $content, $length) {
      $texto = explode(' ', $content, $length);
      if (count($texto)>=$length) {
          array_pop($texto);
          $texto = implode(" ",$texto). $after;
      } else {
          $texto = implode(" ",$texto);
      }
      return $texto;
